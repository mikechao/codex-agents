...
function assertSafeParent(root, path) {
    ...
    try {
        ...
        if (error instanceof Error && error.message === "ERROR_UNSAFE_PATH")
            throw error;
...
function headEntry(root, path) {
    ...
    for (const record of output.split("\0")) {
        ...
        if (entryPath !== path || fields.length !== 3 || fields[1] !== "blob")
            continue;
...
function validateOptions(options) {
    ...
    if ("allowAbsent" in options &&
        typeof options.allowAbsent !== "boolean") {
        throw new Error("ERROR_INVALID_ARGUMENTS");
    }
...
function main() {
    ...
    try {
        ...
        const category = error instanceof Error && /^ERROR_[A-Z_]+$/u.test(error.message)
            ? error.message
            : "ERROR_INTERNAL";
