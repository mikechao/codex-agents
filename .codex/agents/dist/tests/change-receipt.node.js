...
test("absent entries sort with present paths and keep the hash deterministic", () => {
    withRepository((root) => {
        ...
        assert.deepEqual(first.paths.map((entry) => entry.path), ["a.txt", "b.txt", "new/z.txt"]);
        assert.deepEqual(first.paths.find((entry) => entry.path === "new/z.txt"), { path: "new/z.txt", state: "absent", kind: "missing" });
...
skipOnWindows("rejects unsupported Unix-domain socket filesystem objects", async () => {
    ...
    try {
        ...
        commit(root);
        try {
            await new Promise((listenResolve, reject) => {
