...
import { ACCEPTANCE_STATUSES, MAX_CONTRACTS, MAX_DETAIL, MAX_FINDINGS, MAX_PATHS, MAX_TEXT, RESOLUTION_STATUSES, VALIDATION_STATUSES, boundedString, canonicalJson, contractList, evidenceResults, exactKeys, exactPaths, findings, findingIdList, isoNow, objectDigest, optionalText, repairCycle, resolutionMap, revision, role, stringList, userAuthorization, } from "./validation.js";
...
export function roleView(state, actorRole) {
    ...
    if (actorRole === "parent") {
        for (const key of Object.keys(state)) {
            if (ROLE_VIEW_COMMON.includes(key))
                continue;
            if (key === "legacy_evidence")
                continue;
            if (TEMPORARY_COMPATIBILITY_KEYS.includes(key))
                continue;
            view[key] = clone(raw[key]);
        }
    }
    else {
        const extra = actorRole === "reviewer" && state.workflow_type === "review_only"
...
function baseState({ objective, approvedPaths, baseHead, maxRepairCycles, parentWorkflowId = null, workflowType = "change", sourceWorkflowId = null, linkedFindings = [], remediationContext = null, }) {
    return {
        schema_version: SCHEMA_VERSION,
        version: 0, // producer cast; WorkflowVersion is branded
        workflow_id: null,
...
function reviewTarget(value, approvedPaths, repositoryRoot, currentHead, workflowType) {
    ...
    if (args.review_mode === "working_tree") {
        ...
        if (baseRevision !== currentHead)
            fail("ERROR_STALE_BASE", "base HEAD is not current");
        if (args.head_revision !== null) {
            fail("ERROR_INVALID_SHAPE", "working-tree head revision is invalid");
        }
        if (args.include_staged !== true ||
...
export function createState(input, repositoryRoot, currentHead, options = {}) {
    ...
    if (options.internal === true) {
        const args = exactKeys(input, ["objective", "approved_paths", "base_head", "max_repair_cycles", "parent_workflow_id"], "workflow input", ["base_head", "max_repair_cycles", "parent_workflow_id"]);
        ...
        return baseState({
            ...
            baseHead,
            maxRepairCycles,
            parentWorkflowId: optionalText(args.parent_workflow_id, "parent_workflow_id", 100), // brand cast; documented
        });
    }
    const args = exactKeys(input, [
        "workflow_type",
        "objective",
        "approved_paths",
        "acceptance_criteria",
        ...
        "review_target",
        "max_repair_cycles",
    ], "workflow create", ["max_repair_cycles"]);
    ...
    const objective = boundedString(args.objective, "objective");
    const approvedPaths = exactPaths(args.approved_paths, repositoryRoot);
    const target = reviewTarget(args.review_target, approvedPaths, repositoryRoot, currentHead, args.workflow_type);
    const maxRepairCycles = repairCycle(args.max_repair_cycles ?? 2);
    ...
    state.acceptance_criteria = contractList(args.acceptance_criteria, "acceptance_criteria", "AC", "criterion_id");
...
export function submitImplementation(state, input, repositoryRoot, freshReceipt) {
    ...
    const acceptanceResults = evidenceResults(args.acceptance_results, "acceptance", state.acceptance_criteria, "criterion_id", ACCEPTANCE_STATUSES);
    const validationResults = evidenceResults(args.validation_results, "validation", state.validation_requirements, "validation_id", VALIDATION_STATUSES);
...
export function resumeImplementation(state, input) {
    ...
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version", "resume_context"], "implementation resume");
...
export function acceptConcerns(state, input) {
    ...
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version", "user_authorization"], "concern acceptance");
...
export function submitReview(state, input) {
    ...
    const classifications = resolutionMap(args.prior_finding_classifications, priorIds, "prior_finding_classifications");
    ...
    if (blockingFindings.some((item) => item.severity === "P3") ||
    ...
    next.review_receipt = args.review_receipt
        ? JSON.parse(JSON.stringify(args.review_receipt))
        : null;
    if (args.review_status === "APPROVED")
    ...
    if (args.review_status === "CHANGES_REQUESTED")
        next.phase = "REPAIR_REQUIRED";
    return next;
}
export function authorizeRepair(state, input) {
    ...
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version", "finding_ids"], "repair authorization");
...
export function resumeReview(state, input) {
    ...
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version", "resume_context"], "review resume");
...
export function finalizeRepairExhausted(state, input) {
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version"], "repair exhaustion");
    ensurePhase(state, "REPAIR_REQUIRED");
    if (state.repair_cycle < state.max_repair_cycles)
...
export function authorizeCommit(state, authorization) {
    ...
    const args = exactKeys(authorization, ["workflow_id", "capability", "expected_version", "user_authorization"], "commit authorization");
...
export function recordCommit(state, evidence, input) {
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version", "commit_hash"], "commit record");
...
export function prepareCommit(state, input, evidence) {
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version"], "commit preparation");
    ensurePhase(state, "COMMIT_AUTHORIZED");
    const next = clone(state);
    next.commit_preparation = {
        attempt_id: randomUUID(), // documented brand cast
...
export function submitCommitResult(state, input) {
    ...
    if (args.outcome === "committed") {
        ...
        next.commit_result = {
            outcome: "committed",
            commit_hash: args.commit_hash, // producer cast after the regex check
            failure_summary: null,
        };
        next.phase = "COMMITTED";
    }
...
export function retryCommit(state, input) {
    ...
    const args = exactKeys(input, ["workflow_id", "capability", "expected_version", "retry_context"], "commit retry");
...
export function linkedFollowupInput(state, input, repositoryRoot, currentHead) {
    ...
    const linkedFindings = [...state.blocking_findings, ...state.optional_findings].filter((finding) => ids.includes(finding.finding_id));
    return {
        objective: boundedString(args.objective, "objective"),
        approved_paths: exactPaths(args.approved_paths, repositoryRoot),
        acceptance_criteria: args.acceptance_criteria, // raw passthrough to the child
...
export function linkedFollowupChildState(followup) {
    ...
    state.acceptance_criteria = contractList(followup.acceptance_criteria, "acceptance_criteria", "AC", "criterion_id");
    state.validation_requirements = contractList(followup.validation_requirements, "validation_requirements", "VAL", "validation_id");
    return state;
}
...
export function rangeDirtyBaselinePaths(range) {
    if (!range || !Array.isArray(range.paths))
        return [];
    return range.paths
...
export function scopeChangedPaths(initialReceipt, finalReceipt) {
    ...
    for (const entry of initialReceipt.paths) {
        ...
        if (!initialEntry || !finalEntry)
            fail("ERROR_STATE_CORRUPT", "receipt scope is invalid");
        const { state: _initialState, ...initialRest } = initialEntry;
        const { state: _finalState, ...finalRest } = finalEntry;
        if (canonicalJson(initialRest) !== canonicalJson(finalRest))
...
export function migrateV1State(state) {
    ...
    if (v1.schema_version !== 1)
        corrupt();
    if (!Number.isSafeInteger(v1.version) || v1.version < 0)
        corrupt();
    if (typeof v1.workflow_id !== "string" || !/^[0-9a-f-]{36}$/u.test(v1.workflow_id))
        corrupt();
    if (typeof v1.phase !== "string" || !V1_PHASES.includes(v1.phase))
        corrupt();
    if (typeof v1.objective !== "string" ||
        v1.objective.length === 0 ||
        v1.objective.length > 4000)
        corrupt();
    if (typeof v1.base_head !== "string" || !/^[0-9a-f]{40}$/u.test(v1.base_head))
        corrupt();
    if (!isStringArray(v1.approved_paths) ||
    ...
    if (!Number.isSafeInteger(v1.repair_cycle) || v1.repair_cycle < 0 || v1.repair_cycle > 2)
        corrupt();
    if (!Number.isSafeInteger(v1.max_repair_cycles) ||
        v1.max_repair_cycles < 0 ||
        v1.max_repair_cycles > 2)
        corrupt();
    if (v1.parent_workflow_id !== null && typeof v1.parent_workflow_id !== "string")
        corrupt();
    if (v1.implementation_summary !== null && typeof v1.implementation_summary !== "string")
        corrupt();
    if (v1.implementation_status !== null &&
        !["DONE", "DONE_WITH_CONCERNS", "NEEDS_CONTEXT", "BLOCKED"].includes(v1.implementation_status))
        corrupt();
    if (!isStringArray(v1.implementation_changed_paths))
        corrupt();
    if (!isStringArray(v1.implementation_acceptance_evidence))
        corrupt();
    if (!isStringArray(v1.implementation_validation_evidence))
        corrupt();
    if (v1.implementation_receipt !== null && !isObject(v1.implementation_receipt))
        corrupt();
    if (!isStringArray(v1.implementation_known_failures))
        corrupt();
    if (!isObject(v1.finding_resolution_map) || !isObject(v1.prior_finding_classifications))
        corrupt();
    if (!Array.isArray(v1.blocking_findings) || !Array.isArray(v1.optional_findings))
        corrupt();
    if (v1.review_receipt !== null && !isObject(v1.review_receipt))
        corrupt();
    if (v1.commit_authorization !== null && !isObject(v1.commit_authorization))
        corrupt();
    if (v1.commit_result !== null && !isObject(v1.commit_result))
        corrupt();
    if (!isStringArray(v1.repair_authorized_ids))
        corrupt();
    if (!isStringArray(v1.authorized_optional_ids))
        corrupt();
    if (v1.user_authorization_summary !== null && typeof v1.user_authorization_summary !== "string")
        corrupt();
    const changedPaths = [...v1.implementation_changed_paths].sort();
    const approvedPaths = [...v1.approved_paths].sort();
    const phase = v1.phase === "STOPPED_BLOCKED"
    ...
    return {
        ...
        base_head: v1.base_head,
        approved_paths: approvedPaths,
        acceptance_criteria: [],
        validation_requirements: [],
        review_target: {
            review_mode: "working_tree",
            base_revision: v1.base_head,
            head_revision: null,
            approved_paths: approvedPaths,
            include_staged: true,
            include_unstaged: true,
        ...
        parent_workflow_id: v1.parent_workflow_id,
        source_workflow_id: null,
        linked_findings: [],
        remediation_context: null,
        implementation_summary: v1.implementation_summary,
        implementation_status: v1.implementation_status,
        agent_touched_paths: changedPaths,
        scope_changed_paths: changedPaths,
        acceptance_results: [],
        validation_results: [],
        implementation_receipt: v1.implementation_receipt,
        implementation_known_failures: [...v1.implementation_known_failures],
        finding_resolution_map: { ...v1.finding_resolution_map },
        prior_finding_classifications: { ...v1.prior_finding_classifications },
        blocking_findings: v1.blocking_findings,
        optional_findings: v1.optional_findings,
        review_receipt: v1.review_receipt,
        stop_context: null,
        recovery_context: null,
        repair_authorized_ids: [...v1.repair_authorized_ids].sort(),
        concern_acceptance: null,
        commit_authorization: v1.commit_authorization,
        commit_preparation: null,
        commit_result: v1.commit_result,
        ...
        authorized_optional_ids: [...v1.authorized_optional_ids].sort(),
...
const STATE_FINDING_SEVERITIES = new Set(["P0", "P1", "P2", "P3"]);
const GIT_MODES = new Set(["100644", "100755", "120000"]);
...
function nullableBounded(value, max) {
...
function stringArrayShape(value, maxItems, maxLength) {
...
function pathList(value, allowEmpty) {
...
function contractsShape(value, prefix) {
    if (!Array.isArray(value) || value.length > MAX_CONTRACTS)
        corrupt();
    ...
    for (const item of value) {
        ...
        if (typeof id !== "string" || !new RegExp(`^${prefix}-\\d{3}$`, "u").test(id))
            corrupt();
        bounded(item.description, MAX_TEXT);
    }
}
function resultsShape(value, idField, prefix, statuses) {
    if (!Array.isArray(value) || value.length > MAX_CONTRACTS)
        corrupt();
    for (const item of value) {
        if (!isObject(item))
...
function resolutionMapShape(value) {
    if (!isObject(value))
        corrupt();
...
function findingShape(value, expectedBlocking) {
    if (!isObject(value))
        corrupt();
    ...
    if (!STATE_FINDING_SEVERITIES.has(value.severity))
        corrupt();
    if (value.blocking !== (value.severity !== "P3"))
        corrupt();
    if (expectedBlocking !== undefined && value.blocking !== expectedBlocking)
        corrupt();
...
function findingsShape(value, expectedBlocking) {
...
function receiptPathShape(value) {
    if (!isObject(value))
        corrupt();
    ...
    if (state === "absent") {
        ...
        if (value.kind !== "missing" || !GIT_MODES.has(value.mode))
            corrupt();
    }
...
function receiptShape(value) {
    ...
    if (!Array.isArray(value.paths) || value.paths.length > MAX_PATHS)
        corrupt();
    for (const entry of value.paths)
        receiptPathShape(entry);
    sha64(value.overall_scope_hash);
}
function nullableReceipt(value, legacyTolerant) {
    if (value === null || value === undefined)
        return;
    if (legacyTolerant) {
...
function reviewTargetShape(value) {
    if (!isObject(value))
        corrupt();
    checkKeys(value, [
        "review_mode",
...
function stopContextShape(value) {
    ...
    if (value.status === "INCONCLUSIVE") {
        if (value.stopped_from !== "REVIEWING")
            corrupt();
    }
    else {
        if (!IMPLEMENTATION_STATUSES.has(value.status))
            corrupt();
...
function recoveryContextShape(value) {
    if (value === null || value === undefined)
        return;
    if (!isObject(value))
        corrupt();
...
function concernAcceptanceShape(value) {
    if (value === null || value === undefined)
        return;
    if (!isObject(value))
        corrupt();
...
function commitAuthorizationShape(value) {
    if (value === null || value === undefined)
        return;
    if (!isObject(value))
        corrupt();
...
function commitPreparationShape(value) {
    if (value === null || value === undefined)
        return;
    if (!isObject(value))
        corrupt();
...
function commitResultShape(value) {
    if (value === null || value === undefined)
        return;
    if (!isObject(value))
        corrupt();
    if (value.outcome === "committed") {
        ...
        if (value.failure_summary !== undefined && value.failure_summary !== null)
            corrupt();
    }
        ...
        if (value.commit_hash !== undefined && value.commit_hash !== null)
            corrupt();
    }
    else if (value.outcome === "mismatch") {
        checkKeys(value, ["outcome", "mismatch_category"]);
        if (!MISMATCH_CATEGORIES.has(value.mismatch_category))
            corrupt();
    }
    else {
        corrupt();
...
function legacyEvidenceShape(value) {
    if (!isObject(value))
        corrupt();
...
export function validateWorkflowStateV2(value) {
    if (!isObject(value))
        corrupt();
    const actual = Object.keys(value).sort();
    const required = [...V2_STATE_KEYS].sort();
    const legacy = new Set(V2_LEGACY_STATE_KEYS);
    if (actual.some((key) => !required.includes(key) && !legacy.has(key)) ||
    ...
    if (typeof value.workflow_id !== "string" || !/^[0-9a-f-]{36}$/u.test(value.workflow_id))
        corrupt();
    if (value.workflow_type !== "change" && value.workflow_type !== "review_only")
        corrupt();
    if (typeof value.legacy_v1 !== "boolean")
        corrupt();
    ...
    if (!Number.isSafeInteger(value.repair_cycle) || value.repair_cycle < 0 || value.repair_cycle > 2) {
        corrupt();
    }
    if (!Number.isSafeInteger(value.max_repair_cycles) ||
        value.max_repair_cycles < 0 ||
        value.max_repair_cycles > 2) {
        corrupt();
    }
    ...
    if (value.implementation_status !== null &&
    ...
    resultsShape(value.acceptance_results, "criterion_id", "AC", ACCEPTANCE_STATUSES);
    resultsShape(value.validation_results, "validation_id", "VAL", VALIDATION_STATUSES);
    ...
    if (value.legacy_evidence !== undefined)
        legacyEvidenceShape(value.legacy_evidence);
    ...
    return value; // validated producer cast at the persistence boundary
}
