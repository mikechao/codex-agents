...
function changedFields(before, after) {
    ...
    return keys.filter((key) => key !== "version" && canonicalJson(beforeRecord[key]) !== canonicalJson(afterRecord[key]));
}
...
const roleViewForRole = roleView;
export class WorkflowStore {
    root;
    path;
    faultAfterLinkedChildInsert;
    faultAfterMigrationUpdate;
    db;
    closed = false;
    constructor(options = {}) {
    ...
    #ensureOpen() {
        if (this.closed)
            fail("ERROR_STORE_CLOSED", "workflow store is closed");
    }
    ...
    #migrateLegacyRows() {
        ...
        try {
            ...
            for (const row of rows) {
                ...
                const state = parsed; // envelope/before view; migrateV1State revalidates
                if (schema === 1) {
                    ...
                    const result = this.db
                        .prepare("UPDATE workflows SET version = ?, state_json = ?, state_digest = ?, updated_at = ? WHERE workflow_id = ? AND version = ?")
    ...
    #audit(workflowId, version, eventType, actorRole, summary) {
        this.db
            .prepare("INSERT INTO audit_events (workflow_id, version, event_type, actor_role, summary_json, created_at) VALUES (?, ?, ?, ?, ?, ?)")
    ...
    create(input) {
        ...
        try {
            this.db
                .prepare("INSERT INTO workflows (workflow_id, version, state_json, state_digest, parent_capability_hash, implementer_capability_hash, reviewer_capability_hash, committer_capability_hash, created_at, updated_at) VALUES (?, 0, ?, ?, ?, ?, ?, ?, ?, ?)")
                .run(workflowId, JSON.stringify(state), objectDigest(state), hashes.parent, hashes.implementer, hashes.reviewer, hashes.committer, now, now);
    ...
    get(workflowIdValue, actorRole, token) {
        this.#ensureOpen();
    ...
    audit(workflowIdValue, actorRole, token) {
        this.#ensureOpen();
        const id = workflowId(workflowIdValue);
        const row = this.#row(id);
        this.#assertAuth(row, actorRole, token);
        return this.db
            .prepare("SELECT version, event_type, actor_role, summary_json, created_at FROM audit_events WHERE workflow_id = ? ORDER BY event_id")
            .all(id).map((event) => ({
    ...
    #mutate(workflowIdValue, actorRole, token, expected, eventType, action, outcome = null) {
        this.#ensureOpen();
        ...
        try {
            ...
            this.#audit(id, nextVersion, eventType, selectedRole, auditEnvelope(current, next, row.state_digest, {
    ...
    submitImplementation(input) {
        ...
        const eventType = args.status === "DONE" ? "IMPLEMENTATION_SUBMITTED" : "IMPLEMENTATION_STOPPED";
        const outcome = args.status === "DONE"
            ? null
            : IMPLEMENTATION_STOP_PHASES[args.status];
        return this.#mutate(args.workflow_id, "implementer", args.capability, args.expected_version, eventType, (state) => {
    ...
    resumeImplementation(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "parent", args.capability, args.expected_version, "IMPLEMENTATION_RESUMED", (state) => resumeImplementation(state, args));
    }
    acceptConcerns(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "parent", args.capability, args.expected_version, "CONCERNS_ACCEPTED", (state) => acceptConcerns(state, args));
    }
    submitReview(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "reviewer", args.capability, args.expected_version, "REVIEW_SUBMITTED", (state) => {
            ...
            if (args.review_status === "APPROVED") {
                if (state.review_target.review_mode === "working_tree") {
                    ...
                    verifyReviewReceipt(this.root, args.review_receipt, state.approved_paths, state.base_head);
                }
                else {
    ...
    authorizeRepair(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "parent", args.capability, args.expected_version, "REPAIR_AUTHORIZED", (state) => authorizeRepair(state, args));
    }
    resumeReview(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "parent", args.capability, args.expected_version, "REVIEW_RESUMED", (state) => resumeReview(state, args));
    }
    finalizeRepairExhausted(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "parent", args.capability, args.expected_version, "REPAIR_EXHAUSTED", (state) => finalizeRepairExhausted(state, args), "STOPPED_REPAIR_EXHAUSTED");
    }
    authorizeCommit(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "parent", args.capability, args.expected_version, "COMMIT_AUTHORIZED", (state) => {
            if (state.review_target.review_mode !== "working_tree") {
                fail("ERROR_COMMIT_NOT_ALLOWED", "commit authorization requires a working-tree review");
            }
            if (!state.review_receipt)
                fail("ERROR_STALE_RECEIPT", "review receipt is missing");
    ...
    recordCommit(input) {
        ...
        return this.#mutate(args.workflow_id, "committer", args.capability, args.expected_version, "COMMIT_RECORDED", (state) => {
            ...
            const evidence = verifyCommit(this.root, state, args.commit_hash);
            return recordCommit(state, evidence, args);
        }, (next) => next.commit_result.outcome);
    }
    prepareCommit(input) {
        const args = mutationInput(input);
        return this.#mutate(args.workflow_id, "committer", args.capability, args.expected_version, "COMMIT_PREPARED", (state) => {
    ...
    submitCommitResult(input) {
        ...
        return this.#mutate(args.workflow_id, "committer", args.capability, args.expected_version, "COMMIT_RESULT_SUBMITTED", (state) => {
            const next = submitCommitResult(state, args);
            const mismatch = verifyCommitResult(this.root, state, args);
            if (mismatch)
                return commitMismatch(state, mismatch);
            return next;
        }, (next) => next.commit_result.outcome);
    }
    retryCommit(input) {
        ...
        return this.#mutate(args.workflow_id, "parent", args.capability, args.expected_version, "COMMIT_RETRY_AUTHORIZED", (state) => retryCommit(state, args), "retry");
    }
    createLinkedFollowup(input) {
        ...
        try {
            ...
            this.db
                .prepare("INSERT INTO workflows (workflow_id, version, state_json, state_digest, parent_capability_hash, implementer_capability_hash, reviewer_capability_hash, committer_capability_hash, created_at, updated_at) VALUES (?, 0, ?, ?, ?, ?, ?, ?, ?, ?)")
                .run(childId, JSON.stringify(childState), objectDigest(childState), childHashes.parent, childHashes.implementer, childHashes.reviewer, childHashes.committer, now, now);
            this.#audit(childId, 0, "WORKFLOW_CREATED", "parent", auditEnvelope(null, childState, null, { linked_workflow_id: state.workflow_id }));
            const next = { ...state, version: (expectedVersionNumber + 1) };
            const result = this.db
                .prepare("UPDATE workflows SET version = ?, state_json = ?, state_digest = ?, updated_at = ? WHERE workflow_id = ? AND version = ?")
                .run(next.version, JSON.stringify(next), objectDigest(next), now, id, expectedVersionNumber);
            if (result.changes !== 1)
                fail("ERROR_VERSION_CONFLICT", "workflow version is stale");
            this.#audit(id, next.version, "LINKED_FOLLOWUP_CREATED", "parent", auditEnvelope(state, next, row.state_digest, {
