...
export const tools = [
    {
        ...
        description: "Create a change or review-only workflow and return the parent view plus one-time role capabilities.",
        inputSchema: schema({
    ...
    {
        name: "workflow_get",
        description: "Read the authenticated role's least-authority workflow view with its permitted next actions; capabilities are never returned.",
        inputSchema: schema({
            workflow_id: { type: "string" },
            capability: { type: "string" },
    ...
    },
    {
        ...
        description: "Submit complete ID-addressed implementation evidence; DONE advances IMPLEMENTING or REPAIRING to REVIEWING.",
        inputSchema: schema({
            ...common.properties,
            status: {
                type: "string",
    ...
    },
    {
        ...
        description: "Accept DONE_WITH_CONCERNS results with explicit user authorization and enter review.",
        inputSchema: schema({
            ...common.properties,
    ...
    },
    {
        ...
        description: "Submit reviewer findings; approved working-tree reviews must include a current receipt.",
        inputSchema: schema({
            ...common.properties,
    ...
    },
    {
        ...
        description: "Create a fresh linked cycle-0 change workflow that copies exact source findings, remediation context, and parent/source links.",
        inputSchema: schema({
            ...common.properties,
            objective: { type: "string", minLength: 1, maxLength: 4000 },
            approved_paths: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 200 },
    ...
    },
    {
        ...
        description: "Record separate parent/user commit authorization after receipt freshness verification.",
        inputSchema: schema({
            ...common.properties,
            user_authorization: { type: "string", minLength: 1, maxLength: 2000 },
        }, [...common.required, "user_authorization"]),
    ...
    },
    {
        ...
        description: "Verify the fully staged index against the authorized review receipt and prepare a commit binding the exact HEAD, tree, paths, and receipt.",
        inputSchema: schema(common.properties, common.required),
        annotations: {
    ...
    },
    {
        ...
        description: "Submit the outcome of an external commit attempt; a verified commit enters COMMITTED, an unchanged-HEAD failure enters a retryable stop, and any verification mismatch enters a terminal stop.",
        inputSchema: schema({
            ...common.properties,
    ...
    },
    {
        ...
        description: "Authorize a retry after an unchanged-HEAD commit failure; clears the attempt and result and returns to commit authorization.",
        inputSchema: schema({
            ...common.properties,
    ...
    },
    {
        ...
        description: "Migrated-v1 compatibility: record a legacy committer Git result after verifying current HEAD and reviewed content; rejected for new v2 workflows.",
        inputSchema: schema({ ...common.properties, commit_hash: { type: "string", pattern: "^[0-9a-f]{40}$" } }, [...common.required, "commit_hash"]),
        annotations: {
            title: "Record commit",
            readOnlyHint: false,
            destructiveHint: true,
...
export function createServer(store = openStore()) {
    const server = new Server({ name: "workflow-state", version: "1.0.0" }, { capabilities: { tools: {} }, instructions });
