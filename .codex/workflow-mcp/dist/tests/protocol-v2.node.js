...
async function start(root) {
    ...
    assert.ok(stderrStream, "start(): StdioClientTransport must expose the piped stderr stream for the sanitization assertions");
    ...
    const receipt = (paths = ["note.txt"]) => JSON.parse(execFileSync(process.execPath, [
...
function assertSanitizedStderr(stderrText, { capabilities = [], objectives = [], auths = [], findingTexts = [], paths = [] } = {}) {
    ...
    for (const token of stderrText.split(/\s+/).filter(Boolean)) {
        assert.equal(/^[0-9a-f]{64}$/.test(token), false, "stderr contains a 64-hex capability, hash, or digest");
    }
}
test("change workflow drives an external commit to COMMITTED over STDIO", async () => {
    ...
    try {
        ...
        const implemented = await call("workflow_submit_implementation", implementInput(wf.workflow_id, caps, 0, { implementation_receipt: receipt() }));
        ...
        const approved = await call("workflow_submit_review", reviewInput(wf.workflow_id, caps, 1, { review_receipt: receipt(), review_target: target }));
...
test("range review to linked change and approval over STDIO", async () => {
    ...
    try {
        ...
        const range = await call("workflow_create", createInput(git, {
        ...
        const approved = await call("workflow_submit_review", reviewInput(rangeWf.workflow_id, range.capabilities, 0, {
            review_status: "APPROVED",
        ...
        assert.equal(approved.phase, "STOPPED_APPROVED");
        assert.deepEqual((await call("workflow_get", {
            workflow_id: rangeWf.workflow_id,
            role: "parent",
            capability: range.capabilities.parent,
        })).permitted_next_actions, ["workflow_create_linked_followup"]);
        ...
        const implemented = await call("workflow_submit_implementation", implementInput(child.workflow_id, childCaps, 0, {
            summary: "linked child implemented",
            implementation_receipt: receipt(["note.txt"]),
        }));
        assert.equal(implemented.phase, "REVIEWING");
        writeFileSync(join(root, "note.txt"), "child after\n");
        const childApproved = await call("workflow_submit_review", reviewInput(child.workflow_id, childCaps, 1, {
        ...
        assert.deepEqual(parentAudit.map((event) => event.event_type), ["WORKFLOW_CREATED", "REVIEW_SUBMITTED", "LINKED_FOLLOWUP_CREATED"]);
...
test("repair through stop, resume, and exhaustion over STDIO", async () => {
    ...
    try {
        ...
        const created = await call("workflow_create", createInput(git, { objective, max_repair_cycles: 1 }));
        const wf = created.workflow;
        const caps = created.capabilities;
        const target = workingTarget(wf.base_head);
        ...
        const implemented = await call("workflow_submit_implementation", implementInput(wf.workflow_id, caps, 0, { implementation_receipt: receipt() }));
        assert.equal(implemented.phase, "REVIEWING");
        const changes = await call("workflow_submit_review", reviewInput(wf.workflow_id, caps, 1, {
        ...
        const blocked = await call("workflow_submit_implementation", implementInput(wf.workflow_id, caps, 3, {
            status: "BLOCKED",
        ...
        const repaired = await call("workflow_submit_implementation", implementInput(wf.workflow_id, caps, 5, {
            summary: "repaired",
        ...
        const changes2 = await call("workflow_submit_review", reviewInput(wf.workflow_id, caps, 6, {
            review_status: "CHANGES_REQUESTED",
...
test("safe errors over STDIO for role, version, phase, malformed fields, and range commit denial", async () => {
    ...
    try {
        ...
        await assertError("workflow_submit_implementation", { ...validImplement, capability: caps.parent }, "ERROR_CAPABILITY_DENIED");
        await assertError("workflow_submit_implementation", { ...validImplement, expected_version: 99 }, "ERROR_VERSION_CONFLICT");
        await assertError("workflow_submit_implementation", { ...validImplement, changed_paths: [] }, "ERROR_INVALID_SHAPE");
        await assertError("workflow_submit_review", reviewInput(wf.workflow_id, caps, 0, {
        ...
        assert.equal(changes.phase, "REPAIR_REQUIRED");
        await assertError("workflow_authorize_repair", {
            workflow_id: wf.workflow_id,
            capability: caps.implementer,
            expected_version: 2,
            finding_ids: ["DENY-1"],
        ...
        const rangeWf = range.workflow;
        await call("workflow_submit_review", reviewInput(rangeWf.workflow_id, range.capabilities, 0, {
            review_receipt: null,
            review_target: rangeWf.review_target,
        }));
        const rangeAuth = "protocol v2 range denial authorization";
        await assertError("workflow_authorize_commit", {
            workflow_id: rangeWf.workflow_id,
            capability: range.capabilities.parent,
            expected_version: 1,
