...
test("rejects extra mutation fields without changing workflow state", () => {
    ...
    try {
        const store = new WorkflowStore({ repositoryRoot: root, databasePath: join(root, "state.sqlite") });
        assert.equal(errorCategory(() => store.create({ ...createInput(root, git, { objective: "extra" }), extra: true })), "ERROR_INVALID_SHAPE");
...
test("audit history is append-only and versioned across mutations", () => {
    ...
    try {
        ...
        const id = created.workflow.workflow_id;
        const readAudit = () => store.audit(id, "parent", created.capabilities.parent);
        const rawIds = () => store.db.prepare("SELECT event_id FROM audit_events WHERE workflow_id = ? ORDER BY event_id").all(id).map((row) => row.event_id);
...
test("create rejects unknown fields and invalid target combinations", () => {
    ...
    try {
        ...
        assert.equal(errorCategory(() => store.create(createInput(root, git, {
            review_target: {
...
test("implementation evidence requires exact contract IDs in contract order", () => {
    ...
    try {
        ...
        assert.equal(errorCategory(() => submit(validAcceptance, [
            { validation_id: "VAL-002", status: "passed", evidence: "v" },
            { validation_id: "VAL-001", status: "passed", evidence: "v" },
...
test("failed and not-run validation, unsatisfied criteria, and known failures block DONE", () => {
    ...
    try {
        ...
        for (const attempt of attempts) {
            assert.equal(errorCategory(() => implementation(store, created, root, 0, "blocked", {}, "DONE", {
...
test("agent touched paths must be a subset of the approved scope", () => {
    ...
    try {
        ...
        const id = created.workflow.workflow_id;
        const caps = created.capabilities;
        assert.equal(errorCategory(() => implementation(store, created, root, 0, "touched", {}, "DONE", { touched: ["other.txt"] })), "ERROR_INVALID_IMPLEMENTATION");
        assert.equal(store.get(id, "parent", caps.parent).version, 0);
...
test("review-only creation rejects bad flags, revisions, paths, and ancestry", () => {
    ...
    try {
        const store = new WorkflowStore({ repositoryRoot: root, databasePath: join(root, "state.sqlite") });
        const bad = (review_target, options = {}) => store.create(createInput(root, git, {
            objective: "bad range",
            workflow_type: "review_only",
...
test("working-tree approval requires a receipt and range approval rejects receipts", () => {
    ...
    try {
        ...
        const submit = (workflow, capability, target, receiptValue, version = 0) => store.submitReview({
...
test("review submission requires canonical target equality and rejects stale receipts", () => {
    ...
    try {
        ...
        const submit = (target) => store.submitReview({
            workflow_id: id,
...
function authorizedWorkflow(store, root, git, options = {}) {
...
test("commit preparation succeeds across modify, add, delete, and mode and persists exact fields", () => {
    ...
    try {
        ...
        store.close();
        const reopened = new WorkflowStore({ repositoryRoot: root, databasePath: join(root, "state.sqlite") });
...
function insertV1Workflow(path, state) {
  ...
  `);
    ...
    db.prepare("INSERT INTO workflows (workflow_id, version, state_json, parent_capability_hash, implementer_capability_hash, reviewer_capability_hash, committer_capability_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").run(state.workflow_id, state.version, JSON.stringify(state), hashCapability(caps.parent), hashCapability(caps.implementer), hashCapability(caps.reviewer), hashCapability(caps.committer), "2024-01-01T00:00:00.000Z", "2024-01-01T00:00:00.000Z");
    db.close();
    return caps;
}
