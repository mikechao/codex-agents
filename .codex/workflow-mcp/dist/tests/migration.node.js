...
function createV1Database(path, states, audits = []) {
  ...
  `);
    const insertWorkflow = db.prepare("INSERT INTO workflows (workflow_id, version, state_json, parent_capability_hash, implementer_capability_hash, reviewer_capability_hash, committer_capability_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    const insertAudit = db.prepare("INSERT INTO audit_events (workflow_id, version, event_type, actor_role, summary_json, created_at) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    for (const state of states) {
        ...
        insertWorkflow.run(state.workflow_id, state.version, JSON.stringify(state), hashCapability(capabilities.parent), hashCapability(capabilities.implementer), hashCapability(capabilities.reviewer), hashCapability(capabilities.committer), "2024-01-01T00:00:00.000Z", "2024-01-01T00:00:00.000Z");
    }
    for (const event of audits) {
        insertAudit.run(event.workflow_id, event.version, event.event_type, event.actor_role, event.summary_json, event.created_at);
    }
    db.close();
    return issued;
}
...
test("maps STOPPED_BLOCKED phases and preserves other phases", () => {
    ...
    try {
        ...
        const store = new WorkflowStore({ repositoryRoot: root, databasePath: path });
        const phases = states.map((state) => store.get(state.workflow_id, "parent", issued.get(state.workflow_id).parent).phase);
        ...
        assert.equal(store.audit(states[0].workflow_id, "parent", issued.get(states[0].workflow_id).parent).filter((event) => event.event_type === "WORKFLOW_MIGRATED").length, 1);
        store.close();
    }
    finally {
        rmSync(root, { recursive: true, force: true });
...
test("preserves capability hashes and enforces stale versions after migration", () => {
    ...
    try {
        ...
        assert.equal(errorCategory(() => store.get(state.workflow_id, "parent", "bad")), "ERROR_CAPABILITY_DENIED");
        ...
        assert.equal(errorCategory(() => store.authorizeRepair({
...
test("preserves old audits byte-for-byte and appends one migration event", () => {
    ...
    try {
        ...
        assert.equal(events[2].summary.state_digest_after, store.db
...
test("migration audit envelope uses the normative sanitized envelope", () => {
    ...
    try {
        ...
        assert.equal(events[0].summary.state_digest_after, store.db
            .prepare("SELECT state_digest FROM workflows WHERE workflow_id = ?")
            .get(state.workflow_id).state_digest);
        store.close();
    }
...
test("rejects malformed and unknown v1 schemas and rolls back the batch", () => {
    ...
    try {
        ...
        assert.equal(db.prepare("SELECT COUNT(*) AS count FROM audit_events WHERE event_type = 'WORKFLOW_MIGRATED'").get().count, 0);
        db.close();
        const unknownPath = join(root, "unknown.sqlite");
        createV1Database(unknownPath, [v1State({ schema_version: 3 })]);
        assert.equal(errorCategory(() => new WorkflowStore({ repositoryRoot: root, databasePath: unknownPath })), "ERROR_STATE_CORRUPT");
    }
    finally {
        rmSync(root, { recursive: true, force: true });
    }
...
test("reopens a v1 COMMIT_AUTHORIZED fixture via STDIO and completes the allowed legacy path", async () => {
    ...
    try {
        ...
        try {
            ...
            assert.deepEqual(audit.map((event) => event.event_type), ["WORKFLOW_MIGRATED", "COMMIT_RECORDED"]);
