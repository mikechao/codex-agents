...
export function currentHead(root) {
    ...
    if (!/^[0-9a-f]{40}$/u.test(head))
        fail("ERROR_NO_HEAD", "repository has no commit");
    return head;
}
...
function treeEntry(root, revision, path) {
    ...
    if (separator < 0)
        fail("ERROR_INVALID_REVIEW_PATH", "review path metadata is invalid");
...
export function reviewRange(root, target) {
    ...
    const { base_revision, head_revision } = verifyRange(root, target.base_revision, target.head_revision);
    let paths;
    try {
...
function treeEntries(root, revision) {
    ...
    for (const record of output.split("\0")) {
        ...
        if (fields.length !== 3 || fields[1] !== "blob")
            continue;
        result.set(record.slice(separator + 1), {
...
function blobDigest(root, object) {
    ...
    if (!Number.isSafeInteger(size) || size < 0)
        fail("ERROR_GIT", "blob size is invalid");
...
export function verifyReviewReceipt(root, receipt, expectedPaths, baseHead) {
    ...
    if (current.base_head !== baseHead)
        fail("ERROR_STALE_RECEIPT", "receipt base is stale");
...
export function createReceipt(root, expectedPaths, allowAbsent = false) {
    ...
    let current;
    try {
        current = JSON.parse(execFileSync(process.execPath, [script, ...args], {
            cwd: root,
            encoding: "utf8",
        ...
        if (/^ERROR_[A-Z_]+$/u.test(category))
            fail(category, "receipt rejected");
...
export function verifyCommit(root, state, commitHash) {
    ...
    if (parent.length !== 2)
        return { ok: false, mismatch: "PARENT_MISMATCH" };
    ...
    if (actualChanged.length !== expectedChanged.length ||
        actualChanged.some((path, index) => path !== expectedChanged[index])) {
        return { ok: false, mismatch: "PATH_MISMATCH" };
    }
    return { ok: true, commit_hash: commitHash, changed_paths: actualChanged };
}
...
export function stagedEntries(root) {
    ...
    for (const record of output.split("\0")) {
        ...
        if (fields.length !== 3 || fields[2] !== "0")
            continue;
        entries.set(record.slice(separator + 1), {
            mode: normalizeMode(fields[0]),
            object: fields[1],
        });
    }
...
export function writeTree(root) {
    ...
    if (!/^[0-9a-f]{40}$/u.test(tree))
        fail("ERROR_GIT", "tree write failed");
    return tree;
}
export function prepareCommitReceipt(root, state) {
    ...
    if (staged.length === 0)
        fail("ERROR_STAGED_SCOPE", "no paths are staged");
    ...
    for (const entry of receipt.paths) {
        if (entry.state === "deleted") {
            if (entries.has(entry.path))
                fail("ERROR_STAGED_CONTENT", "deleted path is still staged");
            continue;
        }
        ...
        if (!stagedEntry)
            fail("ERROR_STAGED_CONTENT", "changed path is not staged");
...
function commitChangedPaths(root, fromRevision, toRevision) {
    ...
    for (let index = 0; index < parts.length; index += 1) {
        ...
        if (!path)
            fail("ERROR_COMMIT_MISMATCH", "commit path is invalid");
        changed.add(path);
        if (status.startsWith("R") || status.startsWith("C")) {
            const destination = parts[index + 2];
            if (!destination)
                fail("ERROR_COMMIT_MISMATCH", "rename path is invalid");
