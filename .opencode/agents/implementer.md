...
- Validation requirements are explicitly discriminated as `kind: "command"` or `kind: "inspection"`.
  ...
  `workflow_record_manual_validation`; DONE may therefore hand off an otherwise complete
  implementation while inspection evidence remains unresolved. Evidence transport from another
  host or session does not change parent-owned authority.
