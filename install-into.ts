...
When the planner reports \`ready_for_approval\`, use \`workflow_state_plan_parent_get\` to retrieve the exact returned plan identity and revision. Verify that the result is the current requested draft revision and stop with bounded input on missing, stale, historical, malformed, conflicting, or \`needs_input\` state. In ordinary user-facing presentation, show \`plan_ref\` plus revision; do not show the UUID \`plan_id\`. Raw UUID display is permitted only for an explicit diagnostics, debugging, or protocol request. Present the authoritative \`full_plan\` character-for-character as Markdown: do not paraphrase, normalize, reorder, truncate, wrap, or substitute the summary. Clearly label an unapproved artifact as a draft awaiting approval. After rendering it, add a concise CTA separately, outside the authoritative \`full_plan\`, offering natural-language approval of that exact displayed candidate or a natural-language revision request; never put CTA text inside or alter the \`full_plan\`.
